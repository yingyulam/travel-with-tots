...
This file is loaded into the chatbot's system prompt (see
`src/prompts/website_chatbot.txt`) via the `{knowledge_base}` placeholder.
Edit the sections below to add or update what the chatbot knows.
...
lets parents adjust on the fly when the day doesn't go to plan.

Who it's for: parents of young children, especially those visiting
Vancouver and unfamiliar with the city.
...
Parents sign up / log in to save their info. From the dashboard, they
can add, edit, or remove a child's profile (name, age), and reopen any
previously saved trip itinerary. The trip-planning form itself also
lets you enter a child's details on the spot if you haven't saved one
yet.
...
- Account required: yes, to save child profiles and reopen past itineraries
...
- Nearby search categories: kid-friendly restaurant, family room, changing table, nursing room, quiet spot
- Navigation: hands off to Google Maps, no in-app turn-by-turn

## Hours

<!-- e.g. support hours, or "self-serve web app, no live support hours" -->
